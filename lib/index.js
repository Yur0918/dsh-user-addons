...
import { readFileSync, statSync as fspSync } from "node:fs";
...
export function readOwnVersion() {
	try {
		return JSON.parse(readFileSync(path.join(path.dirname(new URL(import.meta.url).pathname), "..", "package.json"), "utf8")).version || "unknown";
	} catch {
		return "unknown";
	}
}

export function emptyFileFold() {
	return {
		title: "",
		firstUser: "",
		createdAt: 0,
...
export function firstTextOf(content) {
	...
	for (const block of content) {
		if (block !== null && typeof block === "object" && block.type === "text" && typeof block.text === "string" && block.text.trim() !== "") return block.text.trim();
	}
	return "";
}

export function foldLine(fold, event) {
	...
	if (type === "user/message" && fold.firstUser === "" && data !== null && typeof data === "object") {
		// untitled sessions fall back to their first real prompt for display
		const text = firstTextOf(data.content);
		if (text !== "" && !text.startsWith("Current runtime context")) fold.firstUser = text.length > 80 ? text.slice(0, 80) + "…" : text;
		return;
	}
...
export function parseZstdFrames(buf) {
	const frames = [];
	let offset = 0;
...
export function decodeZstdLog(buf) {
...
async function sessionLogsUnderRoot(sessionsRoot, logs, seen) {
	let workspaceDirs;
	try {
		workspaceDirs = await fsp.readdir(sessionsRoot, { withFileTypes: true });
	} catch {
		return;
	}
			...
			for (const sessionDir of await fsp.readdir(root, { withFileTypes: true })) {
				...
				for (const candidate of ["session.jsonl.zstd", "session.jsonl"]) {
					...
					try {
						...
						if (stat.isFile() && !seen.has(file)) {
							seen.add(file);
							logs.push({ id: sessionDir.name, file, stat });
						}
						break;
					} catch {
...
async function listSessionLogs() {
	const logs = [];
	const seen = new Set();
	await sessionLogsUnderRoot(path.join(dshHome(), "sessions"), logs, seen);
	// model-route isolated homes spun up by the dsh-mcp bridge live in
	// ~/.dsh-routes/<route>/sessions — their tasks never appear in the main
	// home, so the usage picture would silently miss every dispatched job
	const routesRoot = path.join(os.homedir(), ".dsh-routes");
	let routeDirs;
	try {
		routeDirs = await fsp.readdir(routesRoot, { withFileTypes: true });
	} catch {
		routeDirs = [];
	}
	for (const route of routeDirs) {
		if (!route.isDirectory()) continue;
		await sessionLogsUnderRoot(path.join(routesRoot, route.name, "sessions"), logs, seen);
	}
	return logs;
}
...
function mergeSummary(entries) {
	...
	for (const { id, fold } of entries) {
		if (fold.calls === 0 && fold.title === "" && fold.firstUser === "") continue;
		totals.sessions += 1;
		...
		sessions.push({
			id,
			title: fold.title || fold.firstUser,
...
async function scanAll() {
	...
	const attempted = await Promise.all(logs.map(async ({ id, file, stat }) => {
		try {
	...
	const entries = attempted.filter((entry) => entry !== null);
	return { entries, summary: mergeSummary(entries) };
}

// ── archive helpers ─────────────────────────────────────────────────────────

async function archiveList(query) {
	...
	// server-side search: cheap metadata substring first, then a full-text pass
	// over the decoded log (case-insensitive). Unreadable logs simply fail the
	// full-text arm — they can still match on metadata.
	const q = typeof query === "string" ? query.trim().toLowerCase() : "";
	const index = q !== "" ? await logsByPathIndex() : null;
	const byId = new Map(scans.map(({ id, fold }) => [id, fold]));
	const items = [];
	for (const sessionId of archivedIds) {
		const fold = byId.get(sessionId);
		const title = fold?.title || fold?.firstUser || sessionId;
		const cwd = fold?.cwd || null;
		if (q !== "") {
			const metaHit = title.toLowerCase().indexOf(q) !== -1
				|| sessionId.toLowerCase().indexOf(q) !== -1
				|| (cwd || "").toLowerCase().indexOf(q) !== -1;
			let textHit = false;
			if (!metaHit && index !== null) {
				const entry = index.get(sessionId);
				if (entry !== undefined) textHit = await logTextContains(entry.file, q);
			}
			if (!metaHit && !textHit) continue;
		}
		items.push({
			sessionId,
			title,
			...
			...(q !== "" ? { matchedBy: metaHit ? "meta" : "text" } : {})
		});
	}
	return { ok: true, items };
}

// sessionId → { file } from the same discovery walk the usage scanner uses
// (main home + ~/.dsh-routes), so search/preview see exactly the sessions the
// usage dashboard counts
async function logsByPathIndex() {
	const logs = await listSessionLogs();
	return new Map(logs.map(({ id, file }) => [id, { file }]));
}

async function logTextContains(file, q) {
	try {
		const raw = await fsp.readFile(file);
		const text = file.endsWith(".zstd") ? decodeZstdLog(raw) : raw.toString("utf8");
...
// read-only preview: decodes the session log and returns title, first user
// message, and the last few turns (truncated) — enough to recognize a session
// without restoring it. Never writes anything.
export function previewFromText(text) {
	...
	while (start < text.length) {
		...
		if (event.type === "session/title") {
			const data = event.data;
			if (typeof data === "string" && data.trim() !== "") title = data.trim();
			else if (data && typeof data.title === "string" && data.title.trim() !== "") title = data.title.trim();
			continue;
		}
		if (event.type !== "user/message" && event.type !== "assistant/message") continue;
		const data = event.data;
		if (data === null || typeof data !== "object") continue;
		const textOf = firstTextOf(data.content);
		if (textOf === "" || textOf.startsWith("Current runtime context")) continue;
		if (event.type === "user/message" && firstUser === "") firstUser = textOf.length > 200 ? textOf.slice(0, 200) + "…" : textOf;
		turns.push({ role: event.type === "user/message" ? "user" : "assistant", text: textOf.length > 400 ? textOf.slice(0, 400) + "…" : textOf });
	}
	return { title, firstUser, turns: turns.slice(-7) };
}
...
async function previewSession(sessionId) {
	...
	const index = await logsByPathIndex();
	const entry = index.get(sessionId);
	if (entry === undefined) return { ok: false, error: "session log not found" };
	let fold = null;
	try {
		const stat = await fsp.stat(entry.file);
		fold = await scanFile(entry.file, stat);
	} catch {
		fold = null;
	}
	try {
		const raw = await fsp.readFile(entry.file);
		const text = entry.file.endsWith(".zstd") ? decodeZstdLog(raw) : raw.toString("utf8");
		const { title, firstUser, turns } = previewFromText(text);
		return {
			ok: true,
			sessionId,
			title: title || fold?.title || fold?.firstUser || sessionId,
			firstUser,
			turns,
			calls: fold?.calls ?? null,
			inputTokens: fold?.inputTokens ?? null,
			outputTokens: fold?.outputTokens ?? null,
			models: fold ? [...fold.models.values()].map((m) => `${m.provider}/${m.model}`) : [],
			cwd: fold?.cwd || null
		};
	} catch {
		return { ok: false, error: "session log unreadable" };
	}
}
...
// discover document-parsing plugins installed in the web profile (MinerU today;
// the name list is extensible). Read-only directory check, probed once per
// upload batch and cached — when absent, nothing changes for the user.
let parsePluginCache = null;
function detectParsePlugin() {
	if (parsePluginCache !== null) return parsePluginCache;
	const candidates = [
		"@huanlin/dsh-plugin-mineru",
		"dsh-plugin-mineru",
	];
	const roots = [path.join(dshHome(), "profiles", "web", "node_modules"), path.join(dshHome(), "profiles", "node_modules")];
	for (const root of roots) {
		for (const c of candidates) {
			try {
				const stat = fspSync.statSync(path.join(root, c));
				if (stat.isDirectory()) {
					parsePluginCache = { name: c, tool: "mineru_parse_document" };
					return parsePluginCache;
				}
			} catch {}
		}
	}
	parsePluginCache = null;
	return parsePluginCache;
}
...
async function handleUpload(req, res, query) {
	...
	// second-resolution stamps collide when one prompt drops several files with
	// the same name; a short random tail keeps overwrites impossible
	const unique = `${stamp()}-${randomUUID().slice(0, 4)}`;
	const rel = `dsh-uploads/${unique}-${name}`;
	const target = path.join(dir, `${unique}-${name}`);
	try {
	...
	sendJson(res, 200, { ok: true, path: target, relPath: rel, name, size: body.length, parse: detectParsePlugin() });
}
...
export function apply(pluginCtx) {
	// assemblies without the webserver (e.g. headless profiles reusing this
	// package) have no HTTP surface to attach to; stay dormant, not broken
	if (pluginCtx.webServer === undefined) {
		pluginCtx.logger?.warn?.("dsh-user-addons: webServer unavailable; /addons routes not registered");
		return;
	}
	ctx = pluginCtx;
		...
		handler: async (req, res) => {
			...
			try {
				if (req.method === "GET" && pathname === "/addons/health") {
					sendJson(res, 200, { ok: true, addon: "dsh-user-addons", version: readOwnVersion() });
					return;
				}
				if (req.method === "GET" && pathname === "/addons/archive/list") {
					sendJson(res, 200, await archiveList(query.get("q")));
					return;
				}
				if (req.method === "GET" && pathname === "/addons/archive/preview") {
					sendJson(res, 200, await previewSession(query.get("sessionId") ?? ""));
					return;
				}
