...
`;
		...
		// ── hero-phase fallback: page-level upload rail ───────────────────────
		// DeepSeek Harness renders conversation.composer.dock only inside an open
		// session (footer: !hero && zone !== undefined), so the fresh-app hero
		// screen has no DropDock at all. These page-level overlays are mounted
		// once from apply() and serve every phase: the hint bar shows while a
		// file drag hovers with no dock mounted, and the chip rail mirrors the
		// dock's uploads so drops stay visible on the hero screen. Files dropped
		// before a session exists are saved to disk right away; their path
		// markers queue in pendingHeroMarkers and the next DropDock drains them
		// into the new session's draft.
		let activeDockIntake = null;
		const pendingHeroMarkers = [];
		const heroRail = { hint: null, chips: null };

		function ensureHeroRail() {
			if (typeof document === "undefined") return;
			if (heroRail.hint !== null && document.contains(heroRail.hint)) return;
			heroRail.hint = document.createElement("div");
			heroRail.hint.className = "dua-hero-hint";
			heroRail.hint.textContent = T.heroHint;
			heroRail.hint.style.display = "none";
			heroRail.chips = document.createElement("div");
			heroRail.chips.className = "dua-hero-chips";
			document.body.appendChild(heroRail.hint);
			document.body.appendChild(heroRail.chips);
		}
		function heroHintVisible(on) {
			ensureHeroRail();
			heroRail.hint.style.display = on ? "flex" : "none";
		}
		function clearHeroChips() {
			ensureHeroRail();
			heroRail.chips.textContent = "";
		}
		function heroChipElement(item) {
			const chip = document.createElement("div");
			chip.className = "dua-hero-chip";
			chip.title = item.path !== "" ? item.path : (item.error !== "" ? item.error : item.file.name);
			const badge = document.createElement("span");
			badge.className = "dua-badge";
			badge.style.background = extColor(item.ext);
			badge.textContent = item.ext !== "" ? item.ext.toUpperCase().slice(0, 4) : "FILE";
			const meta = document.createElement("span");
			meta.className = "dua-meta";
			const name = document.createElement("span");
			name.className = "dua-name";
			name.textContent = item.file.name;
			const sub = document.createElement("span");
			sub.className = "dua-sub";
			sub.textContent = item.status === "uploading" ? T.uploading + "…" : item.status === "done" ? T.done : item.status === "error" ? (item.error || T.failed) : humanSize(item.size);
			meta.appendChild(name);
			meta.appendChild(sub);
			const status = document.createElement("span");
			status.className = "dua-status";
			status.dataset.state = item.status;
			status.textContent = { uploading: "⟳", done: "✓", error: "✗" }[item.status] || "";
			const x = document.createElement("button");
			x.className = "dua-x";
			x.type = "button";
			x.setAttribute("aria-label", T.remove + " " + item.file.name);
			x.textContent = "✕";
			chip.appendChild(badge); chip.appendChild(meta); chip.appendChild(status); chip.appendChild(x);
			return chip;
		}
		function renderHeroChips(items) {
			ensureHeroRail();
			heroRail.chips.textContent = "";
			for (const item of items) heroRail.chips.appendChild(heroChipElement(item));
		}
		...
		function DropDock(p) {
			...
			React.useEffect(() => {
				if (activeDockIntake === intake) clearHeroChips();
				else renderHeroChips(items);
				return () => { if (activeDockIntake === null) clearHeroChips(); };
			}, [items]);
			...
			return React.createElement("span", { className: "dua-pop-anchor" },
				...
				open ? React.createElement("div", { className: "dua-pop" },
					...
					(snap.byModel || []).slice(0, 4).map((m) => React.createElement("div", { key: m.provider + "/" + m.model, style: { display: "flex", justifyContent: "space-between", gap: 8 } },
						React.createElement("span", { style: { overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, m.model),
		...
		// ── page-level drag & paste fallback (hero screen etc.) ───────────────
		// Active only while no DropDock is mounted: accepts file drops/pastes
		// anywhere on the page, uploads them to disk immediately, mirrors chips
		// on the hero rail, and queues path markers for the next session draft.
		function installHeroFallback() {
			...
			let depth = 0;
			const enter = (e) => {
				if (activeDockIntake !== null || !hasFiles(e)) return;
				claim(e); depth += 1; heroHintVisible(true);
			};
			const over = (e) => {
				if (activeDockIntake !== null || !hasFiles(e)) return;
				claim(e);
			...
			const leave = (e) => {
				if (activeDockIntake !== null || !hasFiles(e)) return;
				claim(e); depth = Math.max(0, depth - 1);
				if (depth === 0) heroHintVisible(false);
			};
			const drop = (e) => {
				if (activeDockIntake !== null || !hasFiles(e)) return;
				claim(e); depth = 0; heroHintVisible(false);
				const dt = e.dataTransfer;
				heroIntake(dt && dt.files ? Array.from(dt.files) : []);
			};
			const end = () => {
				depth = 0;
				if (activeDockIntake === null) heroHintVisible(false);
			};
			const paste = (e) => {
				if (activeDockIntake !== null) return;
				const target = e.target;
				const inEditor = target !== null && target !== undefined && typeof target.closest === "function" && target.closest('[contenteditable="true"],input,textarea') !== null;
				if (inEditor) return;
				const files = e.clipboardData && e.clipboardData.files;
				if (files && files.length > 0) heroIntake(Array.from(files));
			};
			document.addEventListener("dragenter", enter, true);
			document.addEventListener("dragover", over, true);
			document.addEventListener("dragleave", leave, true);
		...
		function heroIntake(files) {
			if (!files || files.length === 0) return;
			ensureHeroRail();
			for (const f of files) {
				const entry = {
					id: "dua-hero-" + Date.now() + "-" + Math.random().toString(36).slice(2, 7),
					file: f, ext: extOf(f.name), size: f.size,
					status: "uploading", path: "", marker: "", error: ""
				};
				const chipEl = heroRail.chips.appendChild(heroChipElement(entry));
				const xBtn = chipEl.querySelector(".dua-x");
				if (xBtn !== null) xBtn.addEventListener("click", () => {
					entry.aborted = true;
					if (entry.marker !== "") {
						const i = pendingHeroMarkers.indexOf(entry.marker);
						if (i !== -1) pendingHeroMarkers.splice(i, 1);
					}
					chipEl.remove();
				});
				uploadFile(f).then((res) => {
					...
					entry.status = "done";
					entry.path = String(res.path);
					entry.marker = T.markerLine(entry.path, entry.ext !== "" ? entry.ext : (ZH ? "文件" : "file"), humanSize(entry.size));
					pendingHeroMarkers.push(entry.marker);
					if (chipEl !== null) {
						const sub = chipEl.querySelector(".dua-sub");
						if (sub !== null) sub.textContent = T.done;
						const status = chipEl.querySelector(".dua-status");
						if (status !== null) { status.dataset.state = "done"; status.textContent = "✓"; }
						chipEl.title = entry.path;
					}
				}).catch((err) => {
					if (entry.aborted) return;
					entry.status = "error";
					entry.error = err && err.code === "too-large" ? T.tooLarge(100) : (err instanceof Error ? err.message : String(err));
					if (chipEl !== null) {
						const sub = chipEl.querySelector(".dua-sub");
						if (sub !== null) sub.textContent = entry.error;
						const status = chipEl.querySelector(".dua-status");
						if (status !== null) { status.dataset.state = "error"; status.textContent = "✗"; }
					}
				});
			}
		}

		// ── official image rail: keep the remove ✕ visible ────────────────────
		// The official AttachmentRail renders its per-image remove button at the
		// thumbnail's top-right corner but hides it behind `opacity: 0` until
		// hover, which reads as "no way to delete". Extract the hashed class
		// from the official plugin's injected stylesheet at runtime and force
		// the button always-visible; if dsh updates change the shape, this
		// silently no-ops instead of breaking anything.
		function revealOfficialRailRemove() {
			if (typeof document === "undefined") return;
			const TAG = "style[data-plugin-css='@deepseek-ai/dsh-client-ui-attachment/AttachmentRail.module.css']";
			const MARK = "style[data-plugin-css='dsh-user-addons-rail']";
			const tryApply = () => {
				const tag = document.querySelector(TAG);
				if (tag === null) return false;
				const m = tag.textContent.match(/\.([A-Za-z0-9_-]*remove)\{[^}]*opacity:0/);
				if (m === null) return false;
				if (document.querySelector(MARK) === null) {
					const s = document.createElement("style");
					s.dataset.plugin = "dsh-user-addons";
					s.dataset.pluginCss = "dsh-user-addons-rail";
					s.textContent = "." + m[1] + "{opacity:1 !important}";
			...
			if (tryApply()) return;
			let tries = 0;
			const timer = setInterval(() => { if (tryApply() || ++tries > 40) clearInterval(timer); }, 500);
		}
