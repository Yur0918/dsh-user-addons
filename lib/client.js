...
.dua-dock{box-sizing:border-box;width:100%;max-width:var(--dsh-composer-card-max-width);padding:2px 0 8px;display:flex;flex-direction:column;gap:8px}
...
.dua-zone:focus-visible{outline:2px solid #3964fe;outline-offset:2px}
...
.dua-items{display:flex;flex-wrap:wrap;gap:8px}
...
.dua-chip:focus-visible{outline:2px solid #3964fe;outline-offset:2px}
.dua-badge{width:30px;height:30px;border-radius:6px;color:#fff;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;letter-spacing:.02em;flex:none;overflow:hidden}
...
.dua-x:focus-visible{outline:2px solid #3964fe;outline-offset:2px}
...
.dua-retry:focus-visible{outline:2px solid #3964fe;outline-offset:2px}
@keyframes dua-spin{to{transform:rotate(360deg)}}
.dua-section{padding:4px 8px 12px;display:flex;flex-direction:column;gap:16px;font-size:13px;color:var(--dsw-alias-label-primary)}
...
.dua-input{flex:1;border:1px solid var(--dsw-alias-border-l2-darkmode-thin,#c9ced6);background:var(--dsw-specific-input-major,#fff);color:var(--dsw-alias-label-primary);border-radius:10px;padding:8px 12px;font-size:13px;font-family:inherit;outline:none}
.dua-input:focus{border-color:#3964fe}
.dua-input:focus-visible{outline:2px solid #3964fe;outline-offset:2px}
.dua-btn{border:1px solid var(--dsw-alias-border-l2-darkmode-thin,#c9ced6);background:var(--dsw-specific-input-major,#fff);color:var(--dsw-alias-label-primary);border-radius:10px;padding:6px 16px;font-size:13px;cursor:pointer;font-family:inherit}
.dua-btn:hover{background:var(--dsw-alias-interactive-bg-hover)}
.dua-btn:disabled{opacity:.45;cursor:default}
.dua-btn:focus-visible{outline:2px solid #3964fe;outline-offset:2px}
...
.dua-row{display:flex;align-items:center;gap:12px;border:1px solid var(--dsw-alias-border-l1,#e8eaee);border-radius:14px;padding:12px 16px;background:var(--dsw-alias-bg-layer-1,transparent)}
.dua-row-main{flex:1;min-width:0}
.dua-row-actions{display:flex;gap:6px;flex-shrink:0;align-items:flex-start}
.dua-preview{margin-top:10px;border:1px solid var(--dsw-alias-border-l2-darkmode-thin,#c9ced6);border-radius:10px;padding:10px 12px;background:var(--dsw-alias-fill-blank,#fff)}
.dua-preview-head{display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:6px}
.dua-preview-turns{display:flex;flex-direction:column;gap:6px;max-height:280px;overflow:auto}
.dua-preview-turn{display:flex;gap:8px;font-size:12.5px;line-height:1.55;white-space:pre-wrap;word-break:break-word}
...
.dua-tag{font-size:10.5px;color:var(--dsw-alias-label-secondary);border:1px solid var(--dsw-alias-border-l2-darkmode-thin,#c9ced6);border-radius:6px;padding:0 8px;line-height:18px;white-space:nowrap}
.dua-empty{color:var(--dsw-alias-label-tertiary);text-align:center;padding:36px 0;font-size:13px}
.dua-error{color:#d33;padding:8px 12px;background:rgba(211,47,47,.07);border-radius:6px;font-size:12.5px}
.dua-ok{color:#17795e;padding:8px 12px;background:rgba(23,121,94,.08);border-radius:6px;font-size:12.5px}
.dua-cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:8px}
.dua-card{border:1px solid var(--dsw-alias-border-l1,#e8eaee);border-radius:14px;padding:12px 16px}
...
.dua-hero-hint{position:fixed;left:50%;bottom:96px;transform:translateX(-50%);z-index:1500;display:none;align-items:center;gap:8px;border:1.5px dashed #3964fe;background:var(--dsw-alias-bg-layer-2,#fff);color:var(--dsw-alias-label-primary);border-radius:14px;padding:8px 16px;font-size:13px;box-shadow:var(--dsw-shadow-lv3,0 8px 28px rgba(0,0,0,.14));max-width:min(520px,90vw);text-align:center}
...
.dua-hero-chip:focus-visible{outline:2px solid #3964fe;outline-offset:2px}
`;
		...
		function DropDock(p) {
			...
			const uploadOne = React.useCallback((entry, note) => {
				...
				uploadFile(entry.file)
					.then((res) => {
						...
						// when a document-parsing plugin (MinerU) is installed, surface
						// the tool on non-image uploads so users discover the pipeline
						const parseHint = res.parse && res.parse.tool && entry.ext !== "jpg" && entry.ext !== "jpeg" && entry.ext !== "png" && entry.ext !== "gif" && entry.ext !== "webp"
							? (ZH ? " · 可用 " + res.parse.tool + " 解析此文档" : " · parse with " + res.parse.tool)
							: "";
						const fullMarker = marker + parseHint;
						let ins = insertMarker(fullMarker);
						if (!ins) { pendingHeroMarkers.push(fullMarker); ins = true; }
						patchItem(entry.id, ins
							? { status: "done", path: String(res.path), marker: fullMarker, note: note || "" }
			...
			const addNativeImages = React.useCallback((images) => {
				try {
					...
					const conv = deps.conv;
					// draft-attachment APIs were renamed in DSH 0.1.5; prefer the new
					// spellings, fall back to the 0.1.1 line per leg.
					const drafts = typeof conv.createDrafts === "function"
						? conv.createDrafts(deps.sessionId || "", images)
						: conv.createDraftImages(images);
					const s = deps.ensureShell();
					let added = false;
					if (s) {
						if (typeof s.addAttachments === "function") added = s.addAttachments(drafts.map((d) => d.id));
						else if (typeof s.addImages === "function") added = s.addImages(drafts.map((d) => d.id));
					}
					if (!added) {
						if (typeof conv.releaseDraftAttachments === "function") conv.releaseDraftAttachments(drafts);
						else if (typeof conv.releaseDraftImages === "function") conv.releaseDraftImages(drafts);
					}
		...
		function ArchiveSection() {
			...
			return React.createElement("div", { className: "dua-section" },
				...
				items === null
					...
					: shown.length === 0
						...
						: React.createElement("div", { className: "dua-list" }, shown.map((s) => React.createElement("div", { key: s.sessionId, className: "dua-row" },
							...
							React.createElement("div", { className: "dua-row-actions" },
								React.createElement("button", { className: "dua-btn", type: "button", disabled: previewBusy === s.sessionId, onClick: () => openPreview(s.sessionId) }, previewBusy === s.sessionId ? "…" : T.preview),
				...
				previewErr !== "" ? React.createElement("div", { className: "dua-error" }, previewErr) : null,
				preview !== null && preview.ok ? React.createElement("div", { className: "dua-preview" },
					React.createElement("div", { className: "dua-preview-head" },
						React.createElement("strong", null, preview.title || preview.sessionId),
						React.createElement("button", { className: "dua-btn", type: "button", onClick: closePreview }, "✕")),
					preview.cwd ? React.createElement("div", { className: "dua-row-sub" }, preview.cwd) : null,
					preview.calls !== null && preview.calls > 0 ? React.createElement("div", { className: "dua-row-sub" }, preview.calls + " " + T.calls + " · " + humanTokens((preview.inputTokens || 0) + (preview.outputTokens || 0)) + " tok" + (preview.models.length > 0 ? " · " + preview.models.join(", ") : "")) : null,
					React.createElement("div", { className: "dua-preview-turns" },
						(preview.turns || []).map((t, i) => React.createElement("div", { key: i, className: "dua-preview-turn" },
							React.createElement("span", { className: "dua-tag" }, t.role === "user" ? "❯" : "✦"),
							React.createElement("span", null, t.text))))) : null);
		}
			...
			ctx.effect(() => slots.inject("conversation.composer.dock", () => {
				const makeDeps = (sessionId) => ({
					...
					ensureConv() {
						...
						if (c !== undefined && c !== null && (typeof c.createDrafts === "function" || typeof c.createDraftImages === "function")) { this.conv = c; return this.conv; }
						return null;
					},
					ensureShell() {
